...
- Use `tgcli chat stats --chat-id <id> --topic-id <id> --type video --format json` for forum-topic video counts and byte totals. Video notes are separate under `--type video-note`.
